    ...
    public class PagedList<T> : List<T>
    {
        ...
        public PagedList(List<T> items, int count, int pageNumber, int pageSize)
        {
            MetaData = new MetaData
            {
                TotalCount = count,
                PageSize = pageSize,
                TotalPages = (int)Math.Ceiling(count / (double)pageSize)//por ejemplo 1.5 lo transfomra a 2
            };
            AddRange(items); //Agrega los elementos de la coleccion al final de la lista
        }

        public static PagedList<T> ToPagegList(IEnumerable<T> entidad, int pageNumber, int pageSize)
        {
            {
